...
class EccVerifier(Verifier):
  ...
  def __init__(self, public_key: PublicKeyTypes, hasher: Hasher):
  ...
  def verify(self, signature: bytes, message: str | bytes) -> bool:
    ...
    try:
      self.public_key.verify(signature, message_digest, ec.ECDSA(utils.Prehashed(self.hasher.algorithm)))
